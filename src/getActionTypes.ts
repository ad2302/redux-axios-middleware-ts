...
export const getActionTypes = ( action: ActionTypeConfigSingle | ActionTypeConfigMultiple, {
  errorSuffix = ERROR_SUFFIX,
  successSuffix = SUCCESS_SUFFIX,
  cancelSuffix = CANCEL_SUFFIX,
}: Partial<Pick<Options, 'errorSuffix' | 'successSuffix' | 'cancelSuffix'>> = {}): string[] => {
  let types;
  ...
  } else if (typeof (action as ActionTypeConfigMultiple).types !== 'undefined') {
